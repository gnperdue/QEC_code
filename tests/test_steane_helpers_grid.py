'''
Usage:
    python test_steane_helpers_grid.py
'''
import unittest
import random
import logging
...
from circuit_specific.steane_helpers import initialize_steane_grid_conn
from circuit_specific.steane_helpers import steane_grid_conn_phase_correction
from circuit_specific.steane_helpers import steane_grid_conn_bit_correction
...
class TestSteaneGridConnectivityCode(unittest.TestCase):
    """Tests for Steane code functions for the 7+3 qubit system with grid connectivity."""
    ...
    def test_grid_phase_and_bit_flip_error_correction_zero_state(self):
        """
        Test Grid Connected Steane initialization, phase, and bit flip error corrections for
        logical zero
        """
        LOGGER.info(sys._getframe().f_code.co_name) # pylint: disable=protected-access
        random.seed(11)
        # -
        initialized_zero_state = initialize_steane_grid_conn(ZERO_STATE7Q)
    ...
    def test_grid_phase_and_bit_flip_error_correction_one_state(self):
        """
        Test Grid Connected Steane initialization, phase, and bit flip error corrections for
        logical one
        """
        ...
        random.seed(11)
        # -
        initialized_one_state = initialize_steane_grid_conn(ONE_STATE7Q)
    ...
    def test_grid_phase_and_bit_flip_error_correction_superpos_state(self):
        """
        Test Grid Connected Steane initialization, phase, and bit flip error corrections for
        logical superpos
        """
        ...
        random.seed(12)
        # -
        initialized_superpos_state = initialize_steane_grid_conn(SUPERPOS_STATE7Q)
        ...
        corrected_state = steane_grid_conn_phase_correction(bit_phase_error_state)
        corrected_state = ancilla_reset(corrected_state, self.n_ancilla)
        corrected_state = steane_grid_conn_bit_correction(corrected_state)
        corrected_state = ancilla_reset(corrected_state, self.n_ancilla)
